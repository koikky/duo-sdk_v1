...
if PLATFORM == 'gcc':
    ...
    LINKER_SCRIPTS_PATH = r' -L ' + SMALL_CORE + '/board/script/cv180x'

    AFLAGS  = ' -c' + DEVICE + ' -x assembler-with-cpp'
    LFLAGS  = DEVICE + ' -nostartfiles -fms-extensions -ffunction-sections -fdata-sections -Wl,--gc-sections,-Map=rtthread_score.map,-cref,-u,_start -T ' + LINKER_SCRIPTS + LINKER_SCRIPTS_PATH
    CPATH   = ''
    LPATH   = ''
...
DUMP_ACTION = OBJDUMP + ' -D -S $TARGET > rtthread_score.asm\n'
POST_ACTION = OBJCPY + ' -O binary $TARGET ' + OUT_DIR + '/rtthread_score.bin\n' + SIZE + ' $TARGET \n'
POST_ACTION += 'cd ../../.. && bash combine-fip.sh ' + SMALL_CORE + ' ' + OUT_DIR + '/rtthread_score.bin \n'
